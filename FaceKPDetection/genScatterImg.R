...
data.dir <- 'dataset/'
out.scatter.file<- paste0(data.dir, 'outImg/scatter/')

if(!dir.exists(out.scatter.file)){
	dir.create(out.scatter.file, recursive = TRUE)
}

idx = seq(1, ncol(d.train), by = 2)
for(i in idx){
	png(paste(out.scatter.file, (i+1)/2, "_", sub("_x", "", names(d.train)[i]), ".png", sep = ""))
